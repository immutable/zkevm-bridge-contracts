    ...
    Utils
{
    ...
    function testFinaliseQueuedWithdrawalERC20() public {
        ...
        vm.warp(now2);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, amount, block.timestamp, 0);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit RootChainERC20Withdraw(address(token), childERC20Token, alice, bob, amount);
        rootBridgeFlowRate.finaliseQueuedWithdrawal(bob, 0);
    ...
    function testFinaliseQueuedWithdrawalEther() public {
        ...
        vm.warp(now2);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, amount, block.timestamp, 0);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
    ...
    function testFinaliseQueuedWithdrawalsAggregatedERC20() public {
        ...
        address childERC20Token = rootBridgeFlowRate.rootTokenToChildToken(address(token));
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 101, block.timestamp, 0);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 102, block.timestamp, 1);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 103, block.timestamp, 2);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 104, block.timestamp, 3);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 105, block.timestamp, 4);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 106, block.timestamp, 5);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 107, block.timestamp, 6);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 108, block.timestamp, 7);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 109, block.timestamp, 8);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(token), alice, bob, 110, block.timestamp, 9);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
    ...
    function testFinaliseQueuedWithdrawalsAggregatedEther() public {
        ...
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 101, block.timestamp, 0);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 102, block.timestamp, 1);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 103, block.timestamp, 2);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 104, block.timestamp, 3);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 105, block.timestamp, 4);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 106, block.timestamp, 5);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 107, block.timestamp, 6);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 108, block.timestamp, 7);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 109, block.timestamp, 8);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
        emit ProcessedWithdrawal(address(NATIVE_ETH), alice, bob, 110, block.timestamp, 9);
        vm.expectEmit(true, true, true, true, address(rootBridgeFlowRate));
